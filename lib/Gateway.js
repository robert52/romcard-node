...
class Gateway {
  ...
  prepareAuthRequestData(params) {
    return new Promise((resolve, reject) => {
      ...
      requestData['P_SIGN'] = utils.signData(
        requestData,
        constants.TRANSACTION_REQ_FIELDS[constants.TRANSACTION_TYPE_PREAUTH],
  ...
  prepareAuthRecurringData(params) {
    return new Promise((resolve, reject) => {
      ...
      if (!params.recurFreq) {
        return reject(new XError('Recurring requency is required', 'REQUIRED_FIELD'));
      }
      ...
      const requestData = {
        ...
        TERMINAL: this.config.terminalId || '',
        EMAIL: this.config.email || '',
        TRTYPE: constants.TRANSACTION_TYPE_PREAUTH_RECURRING,
        COUNTRY: null,
        MERCH_GMT: null,
        TIMESTAMP: utils.getTimestamp(),
        NONCE: utils.generateNonce(),
        BACKREF: this.config.callbackUrl,
        RECUR_FREQ: params.recurFreq,
        RECUR_EXP: params.recurExp
      };

      const fields = constants.TRANSACTION_REQ_FIELDS[constants.TRANSACTION_TYPE_PREAUTH_RECURRING]
        .concat(
          constants.TRANSACTION_REQ_FIELDS_EXTRA[constants.TRANSACTION_TYPE_PREAUTH_RECURRING]
        );
  ...
  _prepareSaleReversalRequest(params, trType) {
    return new Promise((resolve, reject) => {
      ...
      requestData['P_SIGN'] = utils.signData(
        requestData,
        constants.TRANSACTION_REQ_FIELDS[trType],
        this.config.secretKey
      );
  ...
  parseGatewayResponse(data) {
    return new Promise((resolve, reject) => {
      const expectedFields = constants.TRANSACTION_RES_FIELDS[data.TRTYPE];
      ...
      const dataHash = utils.signData(
        Lazy(data).pick(expectedFields).toObject(),
        expectedFields,
        this.config.secretKey
      );
